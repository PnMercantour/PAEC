-- Jointure entre site zh (zh, defens ou altération) et unité pastorale
-- Une ligne par (up, site) en relation
-- paec.eau_zh_site_up_extra source
CREATE MATERIALIZED VIEW paec.eau_zh_site_up_extra TABLESPACE pg_default AS WITH geo AS (
    SELECT c_unite_pastorale_unp.id,
        c_unite_pastorale_unp.geom
...
-- View indexes:
CREATE INDEX eau_zh_site_up_id_idx ON paec.eau_zh_site_up_extra USING btree (id);
CREATE INDEX eau_zh_site_up_id_site_idx ON paec.eau_zh_site_up_extra USING btree (id_site);